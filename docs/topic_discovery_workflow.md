...
Provider adapters are intentionally separated from topic brief generation. The default path is official/public-frontdoor discovery and user-supplied URLs. Crawling or platform-specific scraping is a low-priority, isolated fallback and must record compliance risk before use.

Adapter config: `configs/topic_discovery/provider_adapters.json`.

Planned provider classes:

- `official_web_search`: official reports, regulator pages, company docs, public datasets.
- `public_video_frontdoor`: YouTube, Bilibili, Douyin, TikTok, X, Xiaohongshu public leads; no download or reuse without rights/privacy review.
- `social_comment_or_trend_adapter`: isolated low-priority fallback; comments/accounts are sensitive and require masking.
- `offline_fixture`: enabled default when internet or rights-cleared material is unavailable.

Every adapter output must include source type, provenance, evidence expected, rights/privacy risk, and whether human review is required before capture or render.
